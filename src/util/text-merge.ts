 ...
 */
function isOverlap(prevTail: string[], currHead: string[], d: number): boolean {
    if (d <= 0 || d > currHead.length) return false;
    const SLACK = 2;
...
const PLACEHOLDER_RE =
    /[([]\s*(?:empty|blank(?:[_ ]?audio)?|silence|silencio|inaudible|no[_ ]?(?:audio|speech|sound)|music|música|sonido|noise|ruido|\.{2,})\s*[)\]]/giu;
 ...
 *  - No termina en punto.
 */
export function finalizeTranscript(text: string): string {
