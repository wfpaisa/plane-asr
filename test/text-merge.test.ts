...
test('tolerates one mishearing inside a long seam (fuzzy >= 0.7)', () => {
    ...
    assert.equal(
        dedupChunkJoin('w uno dos tres cuatro', 'uno XXXX tres cuatro resto', 5),
        'resto'
    );
});
...
test('does not eat the first new word past the seam', () => {
    ...
    assert.equal(
        dedupChunkJoin(
            'sobre el mismo árbol y que parecía',
            'y que parecía brillar más que las demás',
            7
        ),
...
test('drops the overlap even when the ASR inserts a word inside it', () => {
    ...
    assert.equal(
        dedupChunkJoin(
            'una estrella que siempre aparecía',
            'que aparecía sobre el mismo árbol',
            7
        ),
...
test('does not over-drop when only a lone word coincides', () => {
    ...
    assert.equal(
        dedupChunkJoin('caminaba sobre', 'sobre todo esto', 7),
        'todo esto'
    );
    assert.equal(
        dedupChunkJoin('el árbol crecía', 'sobre la colina lejana', 7),
        'sobre la colina lejana'
    );
});
...
test('finalizeTranscript: no leading uppercase, no trailing period', () => {
    assert.equal(
        finalizeTranscript('La última estrella. Cada noche.'),
        'la última estrella. Cada noche'
    );
});

test('finalizeTranscript drops seam periods before a lowercase continuation', () => {
    assert.equal(
        finalizeTranscript('miraba el cielo desde la. ventana de su habitación'),
        'miraba el cielo desde la ventana de su habitación'
    );
    // Un punto seguido de mayúscula es una frase real: se conserva.
    assert.equal(
        finalizeTranscript('brillar más que las demás. Una noche desapareció'),
        'brillar más que las demás. Una noche desapareció'
    );
});
...
test('finalizeTranscript strips trailing "(empty)" and lone periods', () => {
    assert.equal(
        finalizeTranscript(
            'sobre el mismo árbol y que parecía brillar más. (empty)'
        ),
        'sobre el mismo árbol y que parecía brillar más'
    );
