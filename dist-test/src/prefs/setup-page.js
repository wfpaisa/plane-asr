...
export function buildSetupPage(ctx) {
    ...
    const updateStatus = () => {
        ...
        if (activeId === entry.id) {
            ...
            setupButton.sensitive = false;
            statusLabel.label = _('%s is downloaded and active.').format(entry.name);
        }
        else if (present) {
            setupButton.label = _('Use this model');
            setupButton.sensitive = true;
            statusLabel.label = _('%s is already downloaded — click to activate it.').format(entry.name);
        }
        else {
            setupButton.label = _('Setup');
            setupButton.sensitive = true;
            statusLabel.label = _('Downloads %s (%s) and activates it.').format(entry.name, formatSize(file.size_bytes));
        }
    };
    updateStatus();
    ...
    const onProgress = (_obj, modelId, fraction) => {
        if (modelId !== SETUP_MODEL_ID)
            return;
        progressBar.fraction = Math.max(0, Math.min(1, fraction));
    };
    const onComplete = (_obj, modelId) => {
        if (modelId !== SETUP_MODEL_ID || !entry)
            return;
        progressBar.visible = false;
        ctx.settings.set_string(SETTINGS_KEYS.ACTIVE_MODEL_ID, entry.id);
        ctx.settings.set_string(SETTINGS_KEYS.ASR_BACKEND, entry.backend);
        ctx.toast(_('Model downloaded and set as active: %s').format(entry.name));
        updateStatus();
    };
    ...
    ctx.settings.connect(`changed::${SETTINGS_KEYS.ACTIVE_MODEL_ID}`, updateStatus);
