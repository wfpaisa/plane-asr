...
function validateSetup(settings) {
    ...
    if (mode === 'gpu') {
        ...
        if (resolved.source === 'none') {
            problems.push(_('no transcription binary found (downloaded or on PATH)'));
        }
        else {
...
export default class PlaneAsrPreferences extends ExtensionPreferences {
    ...
    fillPreferencesWindow(window) {
        ...
        const cliModeRow = new Adw.ComboRow({
            title: _('Binary mode'),
            subtitle: _('CPU prefers a transcribe-cli found on PATH, falling back ' +
        ...
        const syncCliMode = () => {
            ...
            if (!gpu) {
                ...
                if (resolved.source === 'downloaded') {
                    cliStatusRow.subtitle = _('Using downloaded CPU engine (transcribe-cli, x86_64).').format();
                }
                else if (resolved.source === 'path') {
                    cliStatusRow.subtitle = _('Using transcribe-cli from PATH: %s').format(resolved.path);
                }
                else {
                    cliStatusRow.subtitle = _('No binary found. Download the engine from the ' +
