...
import { findModel, formatSize, loadModelCatalog, modelFilePath, pickFile, resolveModelDir, scanDownloaded, } from '../models/catalog.js';
...
export function buildModelsPage(ctx) {
    ...
    const modelParamsRow = entryRow(_('Model path'), 'e.g. /home/user/models/parakeet-tdt-0.6b-v2-Q8_0.gguf');
    ...
    const refreshActiveModel = () => {
        ...
        activeModelLabel.label = _('No catalog model selected — browse below and click "Use" to select one.');
    };
    refreshActiveModel();
    ...
    const catalogWidgets = [activeModelInfoRow];
    ...
    const initSelectionMode = () => {
        const modelId = ctx.settings.get_string(SETTINGS_KEYS.ACTIVE_MODEL_ID) ?? '';
        const params = ctx.settings.get_string(SETTINGS_KEYS.MODEL_PARAMS) ?? '';
        if (modelId) {
    ...
    ctx.settings.bind(SETTINGS_KEYS.MODEL_PARAMS, modelParamsRow.entry, 'text', Gio.SettingsBindFlags.DEFAULT);
    // -- Búsqueda + filtros (sección de catálogo) -----------------------
    const filterGroup = new Adw.PreferencesGroup({
        title: _('Browse models'),
        description: _('Download GGUF models from HuggingFace. They are verified with ' +
    ...
    const syncActiveModel = () => {
        ...
        const activeQuant = ctx.settings.get_string(SETTINGS_KEYS.QUANT_PREFERENCE) ?? '';
        for (const entry of catalog) {
            rows
                .get(entry.id)
    ...
    syncActiveModel();
    ctx.settings.connect(`changed::${SETTINGS_KEYS.ACTIVE_MODEL_ID}`, syncActiveModel);
    ctx.settings.connect(`changed::${SETTINGS_KEYS.QUANT_PREFERENCE}`, syncActiveModel);
    search.connect('search-changed', applyFilter);
    // Reacciona a las señales de descarga del ModelStore compartido.
    const store = getModelStore();
    const onStarted = (_obj, modelId) => {
        rows.get(modelId)?.updateDownloading(true);
    };
    const onProgress = (_obj, modelId, fraction) => {
    ...
    const onFailed = (_obj, modelId, msg) => {
        const r = rows.get(modelId);
        r?.updatePresence(refreshDownloaded().get(modelId) ?? []);
        r?.updateDownloading(false);
        ctx.toast(_('Download failed: %s').format(`${modelId}: ${msg}`));
    };
    const onCancelled = (_obj, modelId) => {
        const r = rows.get(modelId);
        r?.updatePresence(refreshDownloaded().get(modelId) ?? []);
        r?.updateDownloading(false);
    };
    const onDeleted = (_obj, modelId) => {
        const r = rows.get(modelId);
        if (r) {
            r.updatePresence(refreshDownloaded().get(modelId) ?? []);
            r.refreshPath();
        ...
        if (ctx.settings.get_string(SETTINGS_KEYS.ACTIVE_MODEL_ID) === modelId) {
            ctx.settings.set_string(SETTINGS_KEYS.ACTIVE_MODEL_ID, '');
        }
    ...
    const customGroup = new Adw.PreferencesGroup({
        title: _('Storage'),
        description: _('Directory where downloaded models are stored. You can also drop ' +
    ...
    dirEntry.text = resolveModelDir(ctx.settings.get_string(SETTINGS_KEYS.MODEL_DIR) ?? '');
    ...
    ctx.settings.bind(SETTINGS_KEYS.MODEL_DIR, dirEntry, 'text', Gio.SettingsBindFlags.DEFAULT);
    return page;
}
 ...
 */
function buildModelRow(entry, ctx) {
    ...
    actionButton.connect('clicked', () => {
        ...
        if (isSelectedDownloaded()) {
            ...
            if (file)
                ctx.settings.set_string(SETTINGS_KEYS.QUANT_PREFERENCE, file.quant);
    ...
    nameParamsBox.append(new Gtk.Label({
        label: '-',
        xalign: 0,
    ...
    if (entry.recommended)
        badgeBox.append(badgeIcon('heart-symbolic', _('Favorite'), 'planeasr-icon-heart'));
    if (entry.streaming)
        badgeBox.append(badgeIcon('flash-symbolic', _('Streaming'), 'planeasr-icon-streaming'));
    ...
    const downloadedBadge = badgeIcon('downloaded-symbolic', _('Downloaded'), 'planeasr-icon-downloaded');
    ...
    const updateActive = (active, quant) => {
        isActive = active;
...
function buildSubtitle(entry) {
    const langs = entry.language_count <= 4
...
//# sourceMappingURL=models-page.js.map