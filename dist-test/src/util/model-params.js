 ...
 * completos).
 */
export function ensureModelFlag(params) {
    const trimmed = params.trim();
    if (!trimmed)
        return params;
    const toks = parseArgs(trimmed);
    if (toks.some(t => t === '-m' || t === '--model'))
        return params;
 ...
 */
export function extractExtraFlags(params) {
    const trimmed = params.trim();
    if (!trimmed)
        return '';
    const toks = parseArgs(trimmed);
    ...
    const firstFlagIdx = toks.findIndex(t => t.startsWith('-'));
    if (firstFlagIdx <= 0)
        return ''; // sin banderas, o los parámetros son solo una ruta
