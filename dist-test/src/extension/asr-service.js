...
export var AsrState;
(function (AsrState) {
    AsrState[AsrState["Idle"] = 0] = "Idle";
    AsrState[AsrState["Recording"] = 1] = "Recording";
    AsrState[AsrState["Transcribing"] = 2] = "Transcribing";
})(AsrState || (AsrState = {}));
/**
...
export class AsrService {
    constructor(settings, onChange, transcriberOpts) {
        this._state = AsrState.Idle;
        this._recorder = new Recorder();
        this._converter = new AudioConverter();
     ...
     */
    async _runTranscription(audioPath, isPaste, prune) {
        this._setState(AsrState.Transcribing);
        try {
        ...
        if (getWavDataOffset(srcPath) === null) {
            ...
            try {
                ...
                if (e instanceof NoConverterError) {
                    this._setState(AsrState.Idle, {
                        error: _('No audio converter found. Install ffmpeg, ' +
    ...
    async _runStream(audioPath, session) {
        ...
        // un trozo corto.
        const overlapSamples = Math.min(overlapSeconds * SAMPLE_RATE, chunkSamples - SAMPLE_RATE);
        ...
        const maxOverlapWords = overlapSamples > 0
        ...
        while (!session.cancelled) {
            ...
            if (session.dataOffset === null) {
                ...
                if (session.dataOffset === null) {
                    if (session.ended)
                        break; // nunca produjo un encabezado válido
                    await this._sleep(200);
                    continue;
                }
            }
            const available = wavAvailableSamples(audioPath, session.dataOffset);
            let take;
            ...
            let piece = '';
            try {
                writeWavChunk(audioPath, session.dataOffset, cursor, take, outPath);
            ...
            const emitted = lastFull && maxOverlapWords > 0
                ? dedupChunkJoin(lastFull, piece, maxOverlapWords)
                : piece;
            lastFull = piece;
            if (!emitted)
                continue; // todo el fragmento era solapamiento
            if (isPaste) {
    ...
    _migrateLooseWavs() {
        ...
        let iter;
        try {
            iter = rootFile.enumerate_children('standard::name,standard::type', Gio.FileQueryInfoFlags.NONE, null);
        }
        catch {
        ...
        try {
            ...
            while (info !== null) {
                ...
                if (info.get_file_type() === Gio.FileType.REGULAR &&
                    /\.wav$/i.test(name)) {
                    const src = Gio.File.new_for_path(GLib.build_filenamev([root, name]));
                    const dst = Gio.File.new_for_path(GLib.build_filenamev([dest, name]));
                    try {
