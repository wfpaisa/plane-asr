 ...
 */
function extractTranscription(stdout, stderr) {
    const marker = /^text:[ \t]*(.*)$/m;
...
export class Transcriber {
    ...
    async transcribe(audioPath) {
        ...
        const argv = getBackend(this._settings.get_string('asr-backend') ?? 'transcribe-cli').buildArgv(opts);
        if (!argv[0]) {
            ...
            proc.communicate_utf8_async(null, null, (_self, res) => {
                ...
                try {
                    ...
                    if (this._settings.get_boolean(SETTINGS_KEYS.DEBUG_LOGGING)) {
                        console.warn(`[planeasr] argv=${JSON.stringify(argv)} ` +
                            `success=${ok} ` +
    ...
    async _resolveModelParams() {
        ...
        if (!file)
            return ensureModelFlag(userParams);
        const modelDir = resolveModelDir(this._settings.get_string('model-dir') ?? '');
     ...
     * configuración.
     */
    async _resolveFeatures() {
        const accelerator = (this._settings.get_string(SETTINGS_KEYS.ACCELERATOR) ?? 'auto');
        const language = this._settings.get_string(SETTINGS_KEYS.SELECTED_LANGUAGE);
        ...
        return {
            ...
            translate: this._settings.get_boolean(SETTINGS_KEYS.TRANSLATE_TO_ENGLISH),
            threads,
            initialPrompt: this._settings.get_string(SETTINGS_KEYS.INITIAL_PROMPT) ?? '',
        };
    }
    /**
