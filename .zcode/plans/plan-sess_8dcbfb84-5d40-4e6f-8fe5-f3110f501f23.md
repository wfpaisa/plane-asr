...
if (mode === 'manual') {
    ...
    if (
        this._opts.extensionDir &&
        bundledCliAvailable(this._opts.extensionDir)
    ) {
        ...
        if (!name || !findCliInPath(name)) {
            missing = _(
                'No bundled ASR binary for this architecture. ' +
...
if (mode === 'manual') {
    ...
    cliPath =
        this._opts.extensionDir && bundledCliAvailable(this._opts.extensionDir)
